...
// Gallery data keyed by card index — avoids JSON in HTML attributes
const _galleryData = {};
...
function renderProducts() {
  ...
  // Clear old gallery data before re-render
  Object.keys(_galleryData).forEach(k => delete _galleryData[k]);
  ...
  grid.innerHTML = filtered.map((p, i) => {
    ...
    // Store gallery data by index — avoids JSON/quote issues in HTML attributes
    if (hasImg) {
      _galleryData[i] = { imgs: galleryImgs, name: p.name, wa: waHref };
    }
    const clickAttr = hasImg ? `data-gidx="${i}" style="cursor:pointer;"` : '';
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Event delegation for product gallery — index-based lookup, no JSON-in-attribute issues
  document.addEventListener('click', e => {
    const img = e.target.closest('.pc-image[data-gidx]');
    if (!img) return;
    const idx  = parseInt(img.dataset.gidx, 10);
    const data = _galleryData[idx];
    if (data && data.imgs.length) openGallery(data.imgs, data.name, data.wa);
  });
