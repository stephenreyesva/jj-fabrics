...
function filterCat(cat) {
  ...
  const catOrder = ['', '__new__', 'Ladies Suiting', 'Gents Suiting', 'Accessories'];
  const idx = catOrder.indexOf(cat);
  const pills = document.querySelectorAll('.filter-pill');
  if (idx >= 0 && pills[idx]) pills[idx].classList.add('active');
  renderProducts();
  el('collections')?.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
}
...
function renderProducts() {
  ...
  grid.innerHTML = filtered.map((p, i) => {
    ...
    const galleryImgs = [p.img,p.img2,p.img3,p.img4,p.img5].filter(x=>x&&x.trim());
    const hasImg = p.img && p.img.trim();
    const clickAttr = hasImg ? `data-gallery='${JSON.stringify(galleryImgs)}' data-name="${p.name.replace(/"/g,'&quot;')}" data-wa="${waHref}" style="cursor:pointer;"` : '';
    return `<div class="product-card" style="animation-delay:${i*0.06}s">
      ${isNew ? '<div class="pc-badge-new"><span>NEW</span></div>' : ''}
      ${isSale ? `<div class="pc-badge-sale"><span>−${discPct}%</span></div>` : ''}
      <div class="pc-image" ${clickAttr}>${imgHtml}
        ...
        <div class="pc-footer" style="align-items:flex-end;gap:8px;">
          <div style="flex:1;">
            ${isSale ? `<div class="pc-sale-tag">
              ...
              <div class="pst-red"><div class="pst-label">SALE</div><div class="pst-price">Rs.${Number(p.sale_price).toLocaleString()}</div></div>
              <div class="pst-yellow"><div class="pst-off-pct">-${discPct}%</div><div class="pst-off-label">OFF</div></div>
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Event delegation for product gallery — handles all pc-image clicks safely
  document.addEventListener('click', e => {
    const img = e.target.closest('.pc-image[data-gallery]');
    if (!img) return;
    try {
      const imgs  = JSON.parse(img.dataset.gallery);
      const name  = img.dataset.name  || '';
      const wa    = img.dataset.wa    || '#';
      if (imgs.length) openGallery(imgs, name, wa);
    } catch(err) { console.warn('Gallery click error', err); }
  });
